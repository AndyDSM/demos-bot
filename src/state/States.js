...
guildRouter.keyer(data => {
    try {
        switch (data.key) {
            case 'channelCreate':
                return (data.channel.type === 'text' ? data.channel.guild.id : util.symbol.skipRouter);
            case 'channelDelete':
                return (data.channel.type === 'text' ? data.channel.guild.id : util.symbol.skipRouter);
            case 'channelPinsUpdate':
                return (data.channel.type === 'text' ? data.channel.guild.id : util.symbol.skipRouter);
            case 'channelUpdate':
                return (data.newChannel.type === 'text' ? data.newChannel.guild.id : util.symbol.skipRouter);
            case 'clientUserGuildSettingsUpdate':
            ...
            case 'message':
                return (data.message.channel.type === 'text' ? data.message.guild.id : util.symbol.skipRouter);
            case 'messageDelete':
                return (data.message.channel.type === 'text' ? data.message.guild.id : util.symbol.skipRouter);
                //case 'messageDeleteBulk':
            case 'messageReactionAdd':
                return (data.messageReaction.message.channel.type === 'text' ? data.messageReaction.message.guild.id : util.symbol.skipRouter);
            case 'messageReactionRemove':
                return (data.messageReaction.message.channel.type === 'text' ? data.messageReaction.message.guild.id : util.symbol.skipRouter);
            case 'messageReactionRemoveAll':
                return (data.message.channel.type === 'text' ? data.message.guild.id : util.symbol.skipRouter);
            case 'messageUpdate':
                return (data.newMessage.channel.type === 'text' ? data.newMessage.guild.id : util.symbol.skipRouter);
            case 'presenceUpdate':
                return data.newMember.guild.id;
            case 'roleCreate':
